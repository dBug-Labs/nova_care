# NovaCare Test — Workflow T3: Phase 2 AI Engine & Lab Reports
> Paste TEST_AGENT_MASTER.md first, then this file.
> Dependency: /tmp/test_state.json must exist from T2.

---

## What This Tests
- WF5: Groq streaming nurse chat (Nova)
- WF5: Crisis keyword detection (chest pain → 112)
- WF5: Sentiment analysis on messages
- WF5: Health assessment Q&A flow
- WF5: AI recommendations from patient data
- WF6: Lab report upload (PDF/text)
- WF6: Background AI analysis (OCR + parse + explain)
- WF6: Parsed values with flag classification

---

## Instructions for Agent

### Step 1 — Load state from T2
```bash
cat /tmp/test_state.json
```

### Step 2 — Write and run `/tmp/test_t3_ai.py`
...
async def run():
    if not PAT_TOKEN:
        print("❌ No patient token — run T2 first")
        return
    ...
    if session_id:
        try:
            ...
            chk("WF5 — Chat messages saved to DB", has_user and has_asst,
                f"{len(msgs)} messages (user={'✓' if has_user else '✗'}, assistant={'✓' if has_asst else '✗'})")
        except Exception as e:
...
Creatinine: 0.9 mg/dL"""
    ...
    if report_id:
        print(f"  ℹ  Waiting 10s for background AI lab analysis...")
    ...
    print("  State saved to /tmp/test_state.json")

asyncio.run(run())
```

```bash
python /tmp/test_t3_ai.py
```
...
| FAIL | Fix |
|------|-----|
| Chat 401/403 | JWT expired — re-run T2 to get fresh token |
| No SSE tokens in response | Check `routers/ai_nurse.py` — `StreamingResponse` with `media_type="text/event-stream"` |
| Crisis not detected | Check `CRISIS PROTOCOL` section in `services/ai/prompts.py` NURSE_SYSTEM_PROMPT |
| Lab upload 422 | Field name mismatch — check `report_type` and `report_date` form field names in `routers/lab_reports.py` |
| parsed_values empty | `parse_lab_values()` regex didn't match — add print debug in `lab_reports.py` |
| AI analysis pending after 10s | Background task may not be starting — check `asyncio.create_task()` is called in upload route |
| HbA1c not flagged | Check `patterns` list in `parse_lab_values()` — add `hba1c` pattern |
