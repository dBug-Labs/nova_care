# NovaCare Test — Workflow T5: Phase 4 Doctor Dashboard & Weekly Reports
> Paste TEST_AGENT_MASTER.md first, then this file.
> Dependency: /tmp/test_state.json from T2/T4.

---

## What This Tests
- WF9: Doctor-patient link
- WF9: Doctor patient list (sorted by risk)
- WF9: Doctor patient detail (all 5 data sections)
- WF9: Access control — patient blocked from doctor routes
- WF9: Doctor adds note to patient
- WF10: Weekly report generation trigger
- WF10: Report content — narrative, highlights, concerns, goals
- WF10: PDF generation
- WF10: Share report with doctor
- WF10: Doctor can see shared patient data

---

## Instructions for Agent

Write and run `/tmp/test_t5_doctor.py`:
...
try:
    ...
    print("❌ No test_state.json — run T2 first"); exit(1)
...
async def run():
    if not PAT_TOKEN or not DOC_TOKEN:
        print("❌ Missing tokens — run T2 first"); return
    ...
    try:
        r = await sb_post("doctor_patient_links",
            ...
            DOC_TOKEN)
        ok = r.status_code in (200,201,409)  # 409 = already linked, that's fine
    ...
    try:
        ...
        chk("WF9 — Patient detail has all 6 sections", all_present,
            "  ".join(f"{k}={'✓' if v else '✗'}" for k,v in sections.items()))
    ...
    if weekly_report_id:
        print("  ℹ  Waiting 15s for AI weekly report generation (narrative+highlights+goals+PDF)...")
        ...
        try:
            ...
            if reports:
                ...
                chk("WF10 — AI narrative generated", has_narrative,
                    f"'{rep.get('ai_narrative','')[:60]}...'" if has_narrative else "MISSING — check build_weekly_report()")
                ...
                chk("WF10 — PDF generated + stored", has_pdf,
                    f"url={rep.get('pdf_url','')[:50]}..." if has_pdf else "PDF missing — check reportlab install and generate_pdf_report()")
        except Exception as e:
...
| FAIL | Fix |
|------|-----|
| Doctor patient list empty | `doctor_patient_links` not linked — the test does this via Supabase direct; check RLS on doctor_patient_links table |
| Patient detail missing sections | Check `routers/doctors.py` `get_patient_detail()` — all 6 keys must be in returned dict |
| Access control 200 instead of 403 | Add `require_doctor(user)` call at top of every doctor route function |
| Weekly report narrative missing | `build_weekly_report()` background task failed — check server logs for error |
| PDF url missing | `reportlab` not installed — run `pip install reportlab --break-system-packages` |
| Goals missing | OpenRouter `deep_reasoning` call failed — check OPENROUTER_API_KEY and rate limits |
