# NovaCare Test — Workflow T4: Phase 3 Patient Tracking & Reminders
> Paste TEST_AGENT_MASTER.md first, then this file.
> Dependency: /tmp/test_state.json from T2.

---

## What This Tests
- WF7: Log vitals — all fields, AI analysis
- WF7: Auto-flag critical vitals via DB trigger
- WF7: Log wellness — mood, sleep, water, activity, diet
- WF7: AI daily summary generation
- WF7: Today's summary endpoint (vitals + wellness together)
- WF7: 7-day history with burnout detection
- WF8: Add medicine with schedule + stock
- WF8: Today's medicine schedule generated
- WF8: Log intake (taken) → stock auto-deducts
- WF8: Low stock detection flag
- WF8: Create/list/delete reminders
- WF8: Manual stock refill update

---

## Instructions for Agent

Write and run `/tmp/test_t4_tracking.py`:
...
async def run():
    ...
    try:
        ...
        chk("WF7 — Today summary endpoint", ok and has_v and has_w,
            f"vitals={'✓' if has_v else '✗'} wellness={'✓' if has_w else '✗'} adherence={'✓' if has_m else '✗'}")
    ...
    try:
        ...
        chk("WF7 — 7-day history endpoint", ok,
            f"vitals={vc}, wellness={wc}, burnout_alert_field={'✓' if burnout_key else '✗'}")
    except Exception as e:
    ...
    print("  State saved for T5")

asyncio.run(run())
```

```bash
python /tmp/test_t4_tracking.py
```
...
| FAIL | Fix |
|------|-----|
| Critical vitals not flagged | Migration 007 `flag_critical_vitals` trigger not run — paste it in Supabase SQL Editor |
| AI daily summary missing | Check `routers/vitals.py` wellness route — `filled >= 2` condition and `ai_complete` call |
| Stock not deducted | In `routers/reminders.py` `log_intake` — check `stock_count - 1` update logic |
| low_stock field missing | In `GET /medicines`, add `m["low_stock"] = m.get("stock_count",0) <= m.get("refill_alert_at",5)` |
| Today schedule empty | `medicine_logs` not being created when medicine is added — check `for t in req.schedule_times` loop |
