# NovaCare Test — Workflow T2: Phase 1 Auth & Onboarding
> Paste TEST_AGENT_MASTER.md first, then this file.
> Dependency: T1 (ENV test) must score ≥ 80% before running this.

---

## What This Tests
- WF1: Project scaffold — all routers imported in main.py
- WF2: Supabase schema — auto-BMI trigger, auto-flag trigger
- WF3: Auth — patient signup, signin, doctor signup, signin
- WF4: Onboarding — patient health profile save, doctor profile save, role-based routing

---

## Instructions for Agent

Write `/tmp/test_t2_auth.py` with this content and run it:
...
TS      = int(time.time())
PAT_EMAIL = f"patient_{TS}@test.novacare"
DOC_EMAIL = f"doctor_{TS}@test.novacare"
STATE   = {}
results = []
...
async def run():
    ...
    try:
        ...
        chk("WF1 — All 7 routers registered in main.py", not missing,
            f"Missing routers: {missing}" if missing else "auth,vitals,ai,labs,reminders,doctors,reports ✓")
    ...
    if STATE.get("doc_token") and STATE.get("doc_id"):
        try:
            r = await sb_patch(
                ...
                STATE["doc_token"],
                {"specialty":"cardiology","registration_number":f"REG{TS}","hospital_name":"City Hospital"}
            )
    ...
    with open("/tmp/test_state.json","w") as f:
        json.dump({"pat_token":STATE.get("pat_token",""),
                   "pat_id":STATE.get("pat_id",""),
                   "doc_token":STATE.get("doc_token",""),
                   "doc_id":STATE.get("doc_id","")}, f)

    p = sum(1 for _,ok in results if ok)
    t = len(results)
    print(f"\n  SCORE: {p}/{t} passed ({int(p/t*100) if t else 0}%)")
    print(f"  State saved to /tmp/test_state.json for next test")

asyncio.run(run())
```

```bash
python /tmp/test_t2_auth.py
```
...
## Expected Output
```
✅ PASS — WF1 — All 7 routers registered in main.py
✅ PASS — WF3 — Patient /auth/signup
✅ PASS — WF3 — Patient signin + JWT: id=abc123...
✅ PASS — WF2 — Auto BMI trigger (height=170,weight=80→BMI≈27.7)
✅ PASS — WF2 — Auto critical vitals flag trigger (BP=190/120)
✅ PASS — WF4 — Patient onboarding
✅ PASS — WF3 — Doctor /auth/signup
✅ PASS — WF3 — Doctor signin + JWT
✅ PASS — WF4 — Doctor profile
✅ PASS — WF2 — RLS: patient cannot read other profile
...
| FAIL | Fix |
|------|-----|
| signup 500 error | Check `routers/auth.py` — `supabase.auth.admin.create_user` needs SERVICE_KEY not ANON_KEY |
| BMI is None | Migration 007 `calculate_bmi` trigger not applied — run it in Supabase SQL Editor |
| risk_level not 'critical' | Migration 007 `flag_critical_vitals` trigger not applied — run it |
| RLS not blocking | Migration 001 `CREATE POLICY` statements not run — re-run migration 001 |
| Doctor profile 404 | Migration 001 `doctor_profiles` table not created — check migration ran fully |
